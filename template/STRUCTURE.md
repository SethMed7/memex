...
<memex>/                  TEXT ONLY (markdown + small deterministic scripts). No binaries, ever.
  ...
  scripts/                deterministic tooling: validate · organize · conversations · client (NO LLM calls)
```
...
`scripts/client.ts` (`contextPack`). Run `scripts/validate.ts` to confirm invariants. **Adding config?
